...
model_nm = f'{Product}_{yolo_nm}_{Date}{Time}'  # 학습 결과 저장에 사용할 모델 이름
...
# 2. 모델 학습
model.train(
    data=f'/hdd/datasets/cls_data/{Product}/{Data_nm}',  # 경로(학습 데이터 폴더보다 한 단계 상위 폴더)
    epochs=300,                         # 학습 반복 횟수
    patience=50,                       # 조기 종료를 위한 인내 기간
    batch=256,                           # 배치 크기
    imgsz=224,                          # 입력 이미지 크기
    lr0=0.0005,                         # 초기 학습률
    ...
    dropout=0.3,                       # 드롭아웃 비율
    iou=0.5,                           # IoU 임계값
    exist_ok=True,                      # 기존 결과 폴더가 있으면 덮어쓰기
    project=f'/home/{User}/classification/{Product}/runs/classify',  # 결과
    name=f"{model_nm}",                      # 결과 저장 폴더명
)
...
# 4. 검증 데이터셋 준비
val_dir = f'/hdd/datasets/cls_data/{Product}/{Data_nm}/validation'  # 검증 데이터셋 경로
...
results = model.predict(
    ...
    save_txt=True,
    name=pred_nm,
    project=f'/home/{User}/classification/{Product}/runs/classify/{model_nm}',
)
...
'''# (선택)테스트셋 평가 코드 추가
test_dir = f'/hdd/datasets/cls_data/{Product}/test_2'  # {Data_nm}/test'
    ...
    for class_idx, class_name in enumerate(test_class_names):
        ...
        for img_name in os.listdir(class_dir):
            if img_name.lower().endswith(img_exts):
                ...
                test_y_true.append(class_idx)
    test_results = model.predict(
        test_image_paths,
        save=True,
        save_txt=True,
        name=f"{pred_nm}_test",
        project=f'/home/{User}/classification/{Product}/runs/classify/{model_nm}',
    )
    test_y_pred = []
    for result in test_results:
        if hasattr(result, 'probs'):
            test_y_pred.append(result.probs.top1)
        else:
            max_conf_class = result.boxes.cls[result.boxes.conf.argmax()].item()
            test_y_pred.append(int(max_conf_class))
    test_cm = confusion_matrix(test_y_true, test_y_pred)
    print(f'Confusion Matrix for {yolo_nm} (test):')
    print(test_cm)
    test_report = classification_report(test_y_true, test_y_pred, target_names=test_class_names)
    print(f'Classification Report for {yolo_nm} (test): ')
    print(test_report)'''